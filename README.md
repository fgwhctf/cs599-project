...
DataAgent Java AI 是一个面向企业数据仓库的智能数据分析 Agent，用户可以用自然语言提出业务分析问题，系统自动完成问题理解、元数据召回、SQL 生成、SQL 安全校验、查询执行与结果展示，降低业务人员使用数据分析系统的门槛。

## 方向

方向一：Agentic AI 原生开发。

项目类型：垂直领域 Agent，具体定位为面向企业数据仓库的数据分析师 Agent。

本项目从零构建一个面向电商业务分析场景的智能数据分析 Agent。传统数据分析通常依赖固定报表、人工编写 SQL 或 BI 拖拽配置，业务人员在提出临时分析问题时需要理解数据表结构、指标口径和 SQL 语法。本系统通过 Agentic AI 将自然语言问题转化为可执行的只读 SQL，并以流式进度、SQL 与表格结果返回给用户，使 Agent 承担“业务问题理解 + 数据上下文检索 + 查询生成 + 安全校验 + 结果交付”的数据分析师角色。
...
2. 会话记忆：按 `sessionId` 保存最近 5 轮上下文，支持追问改写。
